...
import { deleteSeatType } from '../../../../services/SeatTypeService';
...
const ModalDeleteMovieType = (props) => {
    ...
    const handleSubmitDelete = async () => {
        let response = await deleteSeatType(+dataDelete.id, token)
        if (response.statusCode === 0) {
            props.setCurrentPage(0)
            props.getListSeatTypePaginate(0)
        ...
        <>
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title>XOÁ LOẠI GHẾ</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    Bạn có muốn xoá loại ghế <b>{dataDelete && dataDelete.name ? dataDelete.name : ''}</b>
                </Modal.Body>
                <Modal.Footer>
