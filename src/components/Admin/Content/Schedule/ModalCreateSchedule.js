...
const ModalCreateSchedule = (props) => {
    const { show, setShow, getListSchedulePaginate } = props;
    ...
    const handleSubmitCreate = async () => {
        ...
        if (isName && isCapacity && isSelectedCinema) {
            let response = await createSchedule(name, capacity, selectedCinema.value, token)
            if (response.statusCode === 0) {
                await getListSchedulePaginate(0)
